...
#define SPERB	(DEV_BSIZE/sizeof(short))
...
typedef struct v7direct	DIRECT;
...
struct bufarea {
	...
	daddr_t	b_bno;
	union {
		char	b_buf[DEV_BSIZE];	/* buffer space */
...
char	*argv[];
{
	...
	struct fstab *fsp, *getfsent();
	char *sbrk(), *namecheck();

	setbuffer(stdin, inbuf, 128);
	setbuffer(stdout, outbuf, 128);
	setlinebuf(stdout);
	sync();
	while(--argc > 0 && **++argv == '-') {
		switch(*++*argv) {
	...
	if (argc)			/* arg list has file names */
		...
		do {
			...
			if (preen == 0 || passno == 1 &&
				...
				tempbase = 0;
				if (blockcheck(fsp->fs_spec) == NO && preen)
					exit(8);
			}
		...
		passno++;
		if (!setfsent())
			errexit("Can't rewind checklist file: %s\n", FSTAB);
...
char *dev;
{
	...
	if(rawflg) {
		if(inoblk.b_dirty)
			bwrite(&dfile,membase,startib,(int)niblk*DEV_BSIZE);
		inoblk.b_dirty = 0;
		if(poolhead) {
			clear(membase,niblk*DEV_BSIZE);
			for(bp1 = poolhead;bp1->b_next;bp1 = bp1->b_next);
			bp2 = &((BUFAREA *)membase)[(niblk*DEV_BSIZE)/sizeof(BUFAREA)];
	...
	if(imax - n_files != superblk.fs_tinode) {
		pwarn("FREE INODE COUNT WRONG IN SUPERBLK");
		if (preen)
			printf(" (FIXED)\n");
		if (preen || reply("FIX") == YES) {
			superblk.fs_tinode = imax - n_files;
			sbdirty();
		}
	}
	flush(&dfile,&fileblk);
	...
	if(sflag || (csflag && rplyflag == 0)) {
		...
		if(freemap)
			...
			for(blk = 0; blk < fmapblk-bmapblk; blk++) {
				...
				copy(bp1->b_un.b_buf,bp2->b_un.b_buf,DEV_BSIZE);
				dirty(bp2);
			}
		}
		badblk = dupblk = 0;
		freeblk.df_nfree = superblk.fs_nfree;
		for(n = 0; n < NICFREE; n++)
			freeblk.df_free[n] = superblk.fs_free[n];
		freechk();
		if(badblk) {
		...
		if(fixfree == 0) {
			...
			else if(n_free != superblk.fs_tfree) {
...
daddr_t blk;
{
	...
	for(dirp = dirblk.b_dir; dirp < &dirblk.b_dir[NDIRECT] &&
		...
		p1 = &dirp->d_name[MAXNAMLEN];
		p2 = &direntry.d_name[MAXNAMLEN];
		while(p1 > (char *)dirp)
			*--p2 = *--p1;
...
char *dev;
{
	...
	imax = ((ino_t)superblk.fs_isize - (SUPERB+1)) * INOPB;
	fmin = (daddr_t)superblk.fs_isize;	/* first data blk num */
	fmax = superblk.fs_fsize;		/* first invalid blk num */
	if(fmin >= fmax || 
		(imax/INOPB) != ((ino_t)superblk.fs_isize-(SUPERB+1))) {
		pfatal("Size check: fsize %ld isize %d",
			superblk.fs_fsize,superblk.fs_isize);
		printf("\n");
		ckfini();
		return(NO);
	}
	...
	mbase = membase;
	if(rawflg) {
		if(msize < (MEMSIZE)(NINOBLK*DEV_BSIZE) + 2*sizeof(BUFAREA))
			rawflg = 0;
		else {
			msize -= (MEMSIZE)NINOBLK*DEV_BSIZE;
			mbase += (MEMSIZE)NINOBLK*DEV_BSIZE;
			niblk = NINOBLK;
			startib = fmax;
		}
	}
	...
	if((off_t)msize < totsz) {
		...
		if(rawflg && (off_t)msize > totsz+DEV_BSIZE) {
			niblk += (unsigned)((off_t)msize-totsz)>>DEV_BSHIFT;
			if(niblk > MAXRAW)
				niblk = MAXRAW;
			msize = memsize - (niblk*DEV_BSIZE);
			mbase = membase + (niblk*DEV_BSIZE);
		}
		poolhead = NULL;
		blkmap = mbase;
...
ginode()
{
	...
	if(rawflg) {
		...
		if(iblk < startib || iblk >= startib+niblk) {
			if(inoblk.b_dirty)
				bwrite(&dfile,mbase,startib,(int)niblk*DEV_BSIZE);
			inoblk.b_dirty = 0;
			if(bread(&dfile,mbase,iblk,(int)niblk*DEV_BSIZE) == NO) {
				startib = fmax;
				return(NULL);
			}
			startib = iblk;
		}
		dp = (DINODE *)&mbase[(unsigned)((iblk-startib)<<DEV_BSHIFT)];
	}
...
dostate(s,flg)
{
	...
	else if((bp = getblk((BUFAREA *)NULL,(daddr_t)(smapblk+(byte/DEV_BSIZE)))) == NULL)
...
daddr_t blk;
{
	...
	else if((bp = getblk((BUFAREA *)NULL,blk+(byte>>DEV_BSHIFT))) == NULL)
		errexit("Fatal I/O error\n");
	else
...
register BUFAREA *bp;
{
	...
	if(bp->b_bno == blk)
		return(bp);
	flush(fcp,bp);
	if(bread(fcp,bp->b_un.b_buf,blk,DEV_BSIZE) != NO) {
		bp->b_bno = blk;
		return(bp);
	}
	bp->b_bno = (daddr_t)-1;
...
{
/*
	if (maxblk != howmany(dp->di_size, DEV_BSIZE))
		printf("POSSIBLE FILE SIZE ERROR I=%u (%ld,%ld)\n\n",
		    inum, maxblk, howmany(dp->di_size,DEV_BSIZE));
*/
...
ifreechk() {
	...
	for (i=0; i<superblk.fs_ninode; i++) {
		inum = superblk.fs_inode[i];
		switch (getstate()) {
...
makefree()
{
	...
	superblk.fs_fmod = 0;
	superblk.fs_tfree = 0;
	superblk.fs_ninode = 0;
	superblk.fs_ilock = 0;
	superblk.fs_ronly = 0;
	if(cylsize == 0 || stepsize == 0) {
		step = superblk.fs_step;
		cyl = superblk.fs_cyl;
	}
	else {
		step = stepsize;
	...
	superblk.fs_nfree = freeblk.df_nfree;
	for(i = 0; i < NICFREE; i++)
		superblk.fs_free[i] = freeblk.df_free[i];
	sbdirty();
}
...
register DIRECT *dirp;
{
	...
	for(p1 = dirp->d_name,p2 = srchname;*p2++ == *p1; p1++) {
		if(*p1 == 0 || p1 == &dirp->d_name[MAXNAMLEN-1]) {
...
linkup()
{
	...
	if(dp->di_size & DEV_BMASK) {
		dp->di_size = roundup(dp->di_size,DEV_BSIZE);
		inodirty();
	}
	filsize = dp->di_size;
	inum = orphan;
