...
char *argv[];
{
	...
	bmapest(clrmap);
	bmapest(nodmap);
	etapes = esize / (density * tsize / DEV_BSIZE) + 1;
...
l3:
	lseek(df, (long)n*sizeof(idbuf), L_SET);
	for(i=0;; i++) {
...
struct dinode *ip;
{
	...
	esize++;
	s = (ip->di_size + DEV_BSIZE-1) / DEV_BSIZE;
	esize += s;
	if(s > NADDR-3) {
		s -= NADDR-3;
		s = (s + (DEV_BSIZE/sizeof(daddr_t))-1) / (DEV_BSIZE/sizeof(daddr_t));
		esize += s;
	}
}
...
short *map;
{
	...
	if(n < 0)
		return;
	esize++;
	esize += (n + (DEV_BSIZE/sizeof(short))-1) / (DEV_BSIZE/sizeof(short));
}
